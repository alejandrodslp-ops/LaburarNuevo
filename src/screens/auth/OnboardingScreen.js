...
</>}{i<SLIDES.length-1&&(<TouchableOpacity style={ss.skip} onPress={saltar}><Text style={ss.skipTxt}>{t('saltar')}</Text></TouchableOpacity>)}{!(i===0&&role==='worker')&&<View style={ss.bubble}><Text style={ss.emoji}>{s.emoji}</Text></View>}<View style={ss.tw}><Text style={ss.titulo}>{s.titulo}</Text><Text style={ss.desc}>{s.desc1||s.desc}</Text>{s.promo&&<Text style={ss.promo}>{s.promo}</Text>}{s.desc2&&<Text style={ss.desc}>{s.desc2}</Text>}</View></SafeAreaView></LinearGradient>))}
</ScrollView>
...
promo:{fontSize:16,fontWeight:'900',color:'#FFF5E0',textAlign:'center',marginTop:14,marginBottom:10,letterSpacing:0.2},
