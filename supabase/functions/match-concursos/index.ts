...
// ─────────────────────────────────────────────────────────────
// Expande un keyword canónico en español a todos sus equivalentes
...
// Palabras que solas NO constituyen un match válido contra un keyword compuesto
// del perfil. Incluye roles genéricos y adjetivos/modificadores que aparecen
// en muchos cargos sin definir la profesión específica.
...
//    traducidas al idioma del país del concurso → hasta 80 pts
...
function calcularScore(
  ...
  // Por cada keyword canónico, generamos el par [español, idioma_local]
  // y normalizamos. Guardamos la clave canónica (español) como referencia.
  ...
  // Keywords del concurso (campo keywords precalculado + cargo normalizado)
  ...
  for (const { kw: kp, canonico } of perfilKws) {
    if (matched.includes(canonico)) continue; // ya matcheó por otra variante
    const kpWords = kp.split(" ").filter((w: string) => w.length > 3);
    ...
    for (const kc of concursoKws) {
      ...
      // Si el job keyword es genérico ("auxiliar", "tecnico"…) no alcanza solo
      // para hacer match contra un keyword compuesto del perfil
      const bloqueado = perfilContieneJob && esCompuesto && ROLES_GENERICOS.has(kc);

      if ((exactoOJobEspecifico || perfilContieneJob) && !bloqueado) {
        if (!matched.includes(canonico)) matched.push(canonico);
        break;
      }
    }
  }

  // Para el score usamos el total de keywords únicos del perfil (sin variantes)
  ...
  // Requiere al menos el 50% de los keywords del perfil matcheados Y score ≥ 55
  const ratioOk = matched.length > 0 && totalUnicos > 0 && (matched.length / totalUnicos) >= 0.5;
  const cumple = ratioOk && score >= 55;
