...
const PROXY     = 'https://www.konexu.app/api/proxy?url=';
const PROXY_SEC = process.env.PROXY_SECRET ?? '';
...
router.post('/', async (req, res) => {
  ...
  for (const { codigo, url } of PAISES) {
    ...
    let dbError = null;
    if (total !== null) {
      const { error } = await db.from('mercado_stats').upsert(
  ...
  return res.json({ fecha: hoy, ok, total: resultados.length, resultados, rubros: rubrosResultados });
});
